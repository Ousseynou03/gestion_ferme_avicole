...
import { VeterinaireService } from 'src/app/demo/services/veterinaire.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-edit-veto',
  templateUrl: './edit-veto.component.html',
  styleUrls: ['./edit-veto.component.scss']
})
export class EditVetoComponent {
  ...
  public matDialogRef: MatDialogRef<EditVetoComponent>,
  ) {
     ...
     ngOnInit(): void {
      ...
      if (this.editData) {
        ...
        this.form = this._formBuilder.group({
          ...
          nomVeterinaire: new FormControl(this.editData?.nomVeterinaire,Validators.required),
          traitement: new FormControl(this.editData?.traitement,Validators.required),
          posologie: new FormControl(this.editData?.posologie,Validators.required),
    ...
    updateVeterinaire(){
      ...
      this.veterinaireService.updateVeterinaire(this.editData?.id, fiche, this.headers).subscribe(() => {
        Swal.fire({
          icon: 'success',
          title: 'Succès',
          text: 'Le Veterinaire a été modifié avec succès.'
        });
        ...
        (error)=> {
          Swal.fire({
            icon: 'error',
            title: "Erreur",
            text: 'Impossible de modifier le Veterinaire '+this.editData?.designation
          });
        });
      }else{
        Swal.fire(
