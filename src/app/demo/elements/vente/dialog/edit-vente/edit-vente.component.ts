...
export class EditVenteComponent {
  ...
  public matDialogRef: MatDialogRef<EditVenteComponent>,
  ) {
    ...
    updateVente(){
      ...
      fiche.client=fiche.client.id
      fiche.tresorerie=fiche.tresorerie.id
      console.log("fiche {}", fiche)
  
      this.venteService.updateVente(this.editData?.id, fiche, this.headers).subscribe(() => {
        Swal.fire({
          icon: 'success',
          title: 'Succès',
          text: 'La Vente a été modifié avec succès.'
        });
        ...
        (error)=> {
          Swal.fire({
            icon: 'error',
            title: "Erreur",
            text: 'Impossible de modifier la Vente '+this.editData?.designation
          });
        });
      }else{
        Swal.fire(
